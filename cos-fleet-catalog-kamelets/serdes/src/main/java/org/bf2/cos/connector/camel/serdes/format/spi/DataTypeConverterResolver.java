...
package org.bf2.cos.connector.camel.serdes.format.spi;
...
public interface DataTypeConverterResolver {
    ...
    /**
     * Attempts to resolve the converter for the given scheme and name. Usually uses the factory finder URI to resolve the
     * converter.
     * Scheme and name may be combined in order to resolve component specific converters. Usually implements a fallback
     * resolving mechanism when no matching converter for scheme and name is found (e.g. search for generic Camel converters
     * just using the name).
     *
    ...
    /**
     * Attempts to resolve default converter for the given name. Uses default Camel scheme to resolve the converter via factory
     * finder mechanism.
     *
     * @param  name         the data type name.
     * @param  camelContext the current Camel context.
