...
package org.bf2.cos.connector.camel.serdes.format;
...
import org.bf2.cos.connector.camel.serdes.bytes.ByteArrayDataType;
import org.bf2.cos.connector.camel.serdes.format.spi.DataTypeConverter;
import org.bf2.cos.connector.camel.serdes.format.spi.DataTypeConverterResolver;
import org.bf2.cos.connector.camel.serdes.format.spi.DataTypeLoader;
import org.bf2.cos.connector.camel.serdes.format.spi.DataTypeRegistry;
import org.bf2.cos.connector.camel.serdes.format.spi.annotations.DataType;
import org.bf2.cos.connector.camel.serdes.text.StringDataType;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

/**
 * Default data type registry able to resolve data types converters in the project. Data types may be defined at the component
 * level
 * via {@link DataType} annotations. Also, users can add data types directly
...
public class DefaultDataTypeRegistry extends ServiceSupport implements DataTypeRegistry, CamelContextAware {
    ...
    protected void doInit() throws Exception {
        ...
        LOG.debug("Loaded {} schemes holding {} data type converters", dataTypeConverters.size(),
    ...
    private Optional<DataTypeConverter> getDataTypeConverter(String scheme, String name) {
        if (dataTypeConverterResolver == null) {
            dataTypeConverterResolver = Optional
                    .ofNullable(camelContext.getRegistry().findSingleByType(DataTypeConverterResolver.class))
        ...
        if (dataTypeConverter.isPresent()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Found data type {} for scheme {} and name {} in Camel registry",
        ...
        if (dataTypeConverter.isPresent()) {
            if (LOG.isDebugEnabled()) {
                LOG.debug("Found data type {} for scheme {} and name {}", ObjectHelper.name(dataTypeConverter.get().getClass()),
                        scheme, name);
            }
            return dataTypeConverter;
        }
        ...
        if (LOG.isDebugEnabled() && dataTypeConverter.isPresent()) {
            LOG.debug("Resolved data type {} for scheme {} and name {} via resource path",
                    ObjectHelper.name(dataTypeConverter.get().getClass()), scheme, name);
        }
